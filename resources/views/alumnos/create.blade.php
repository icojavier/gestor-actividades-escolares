...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Registrar Nuevo Alumno</h1>
</div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('curso_academico') is-invalid @enderror" 
                            ...
                            @foreach($cursos as $curso)
                                <option value="{{ $curso }}" {{ old('curso_academico') == $curso ? 'selected' : '' }}>
                                    {{ $curso }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('edad') is-invalid @enderror" 
                               id="edad" name="edad" value="{{ old('edad') }}" min="6" max="18" required>
                        @error('edad')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <div class="form-text">La edad debe estar entre 6 y 18 años.</div>
                    </div>
                </div>
            </div>
